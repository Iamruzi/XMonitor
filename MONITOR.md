...
- 总览：管理监控用户和事件中心
- 按分组：点击每个分组，查看这个组监控了哪些用户，以及组内热门共同关注对象
- 项目雷达：从已采集的关注列表里统计“共同关注”，默认展示所有被多人共同关注的账号，
  也可以切换“只看项目账号”
...
跨组共识、近期集中关注等信号，帮助优先看低粉但被多个观察对象同时关注的项目。项目卡会显示关注时间线，
按发现关注的先后列出是谁在什么时候关注；雷达列表可以切换“热度排序”和“时间线排序”。
项目账号被 2 个监控用户共同关注时标为“注意”；超过 2 个后开始显示 `🔥`，每多 1 个共同关注多 1 个火苗；
超过 5 个共同关注时改用数字和“超级火爆 / 更多关注”提示，避免卡片被火苗刷屏。
新添加的监控用户第一次建立关注基线时，会按 `MONITOR_INITIAL_FOLLOWING_FETCH_COUNT` 拉取更大的关注范围，
默认 200 个，用来补齐“这个新用户其实也关注了已有热门项目”的关系。已经建立过基线的用户可以在表格里点“补齐”，
重新按这个范围写入关注关系但不推送历史事件。这个能力受 X 关注列表接口限制：只能识别被本次拉取到的关注对象，
不能直接对无限历史做精确反查。
...
前端设置页可以分别发送 Telegram、WxPusher、Bark 测试通知，也可以一次测试全部渠道。
